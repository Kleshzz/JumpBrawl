...
#            self.writeScId(29, int(i))
        ...
        self.writeString("brawlstars://extlink?page=https%3A%2F%2Ft.me%2bosinnbrawlik")
        self.writeVint(3473)
        ...
        spgList = []  # Список доступных звездных способностей и гаджетов
        for brawler_id, level in self.player.brawlerPowerLevel.items():
            if level >= 8:  # Уровень бравлера должен быть 8 или выше
                unlocked_spg = Cards.get_spg_id(int(brawler_id))  # Получаем ID способностей и гаджетов
                for spg_id in unlocked_spg:
                    if str(spg_id) in self.player.brawlers_spg_unlock:  # Проверяем наличие в словаре brawlers_spg_unlock
                        if self.player.brawlers_spg_unlock[str(spg_id)] == 1:  # Проверяем, что статус равен 1
                            spgList.append(spg_id)  # Добавляем в список доступных

        # Записываем количество всех навыков (гаджеты + звездные способности)
        self.writeVint(len(self.player.card_skills_id))

        # Записываем данные о каждом навыке
        for skill_id in self.player.card_skills_id:
            self.writeScId(23, skill_id)  # ID навыка (гаджета/способности)
            if skill_id in spgList:  # Если навык в списке доступных
