...
class FriendListMessage(Writer):
    ...
    def encode(self):
        try:
            ...
            if user is None:
                logger.warning(f"Пользователь с lowID {self.player.low_id} не найден.")
                return
            ...
            for data in friends:
                ...
                if self.players is None:
                    logger.warning(f"Игрок с ID {data['id']} не найден. Удаляем из списка друзей.")
            ...
            if valid_friends != friends:
                ...
                conn.commit()
                logger.info("Обновлённый список друзей успешно сохранён.")
            else:
                # Логируем только один раз при первом обнаружении, что изменений нет
                if not hasattr(self, 'no_changes_logged'):
                    self.no_changes_logged = True

        except Exception as e:
            logger.error(f"Ошибка при кодировании FriendListMessage для игрока с lowID {self.player.low_id}: {e}")
        finally:
            conn.close()