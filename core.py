...
class Server:
    ...
    def __init__(self, ip: str, port: int):
        ...
        log_info(f'Lobby Started! {self.ip}:{self.port}')
            ...
            with addr_lock:
                ...
                if address[0] in self.blocked_ips:
                    if address[0] not in self.reported_blocked_ips:
                        log_info(f"[AntiDDoS] Блокировка IP {address[0]} - России.")
    ...
    def country_block(self, ip):
        # Примерная логика блокировки IP для России
        ip_ranges = [
            (ip_to_int("5.0.0.0"), ip_to_int("5.255.255.255")),
            (ip_to_int("31.128.0.0"), ip_to_int("31.255.255.255")),
            (ip_to_int("37.0.0.0"), ip_to_int("37.255.255.255")),
        ]
        ip_int = ip_to_int(ip)
...
class ClientThread(Thread):
    ...
    def run(self):
        ...
        try:
            client_ip = self.client.getpeername()[0]  # Получаем IP-адрес клиента
            while True:
            ...
            self.client.close()
            connected_ips.discard(client_ip)  # Удаление IP из списка подключенных
            log_info(f"[INFO] IP {client_ip} Закрыт")

if __name__ == "__main__":
    server = Server("0.0.0.0", 20594)
    server.start()