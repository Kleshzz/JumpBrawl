        ...
        if self.Road==10:
            ...
            if self.Season in [1]:
                if self.Level in [2,4,8,10,14,23,34,46]:
                    ...
                    brawler = Help().GetAmountOfLevel(self.Season+1, [2], [3]) #Бравлер Айди (брок)
                    skin = Help().GetAmountOfLevel(self.Season+1, [2], [201]) #Скин айди (суперрейнджер)
                    ...
                    brawler = Help().GetAmountOfLevel(self.Season+1, [2], [38]) #Бравлер Айди (вольт)
                    skin = Help().GetAmountOfLevel(self.Season+1, [2], [203]) #Скин айди (паладин)
                    if self.player.UnlockedSkins[str(skin)] == 0:
                        amount = 1
                        self.Skin = [29,skin]
                        self.Character = [16,brawler]
